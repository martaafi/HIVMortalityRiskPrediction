...
Original file is located at
    https://colab.research.google.com/drive/1do8yhXAD3mk6jHDmCoqt_m-_hP9WX4I2
"""
...
class Categorization(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        for col in self.cols:
            ...
            self.bins_[col] = [-np.inf, q1, q3, np.inf]
        return self
    ...
    def transform(self, X):
        ...
        for col in self.cols:
            ...
            X_copy[new_col] = pd.cut(
                X_copy[col], bins=self.bins_[col], labels=['Low', 'Med', 'High']
            )
...
class FeatureConstructor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_copy['Burden_Score'] = (
            X_copy['DALYs'] *
            X_copy['Incidence Rate (%)'] *
            X_copy['Population Affected']
        )

        X_copy['Access_Need_Ratio'] = (
            X_copy['Healthcare Access (%)'] / (X_copy['Prevalence Rate (%)'] + 1e-6)
        )

        X_copy['Severity_Score'] = (
            (100 - X_copy['Recovery Rate (%)']) +
            (100 - X_copy['Healthcare Access (%)']) +
            X_copy['Prevalence Rate (%)']
        )
...
st.title("🩺 HIV Mortality Risk Prediction Dashboard")
st.sidebar.title("🧭 Navigation")

mode = st.sidebar.radio("Choose mode:", ["Model Analysis & Visualization", "Predict Single Patient"])

if mode == "Predict Single Patient":
    st.header("🧪 Predict Mortality Risk for a New Patient")

    # Input fields untuk variabel independen yang dibutuhkan model
    age_group = st.selectbox("Age Group", ["0-18", "19-35", "36-60",  "61+"])
    gender = st.selectbox("Gender", ["Other", "Female", "Male"])
    income = st.number_input("Per Capita Income (USD)", min_value=0.0)
    education_index = st.number_input("Education Index", min_value = 0.0)
    prevalence = st.slider("Prevalence Rate (%)", 0.0, 100.0, 1.0)
    incidence = st.slider("Incidence Rate (%)", 0.0, 100.0, 1.0)
    access = st.slider("Healthcare Access (%)", 0.0, 100.0, 1.0)
    doctors = st.number_input("Doctors per 1000", min_value=0.0, step=0.1)
    affected = st.number_input("Population Affected", min_value=0.0, step=1.0)
    category = st.selectbox("Disease Category", ["Chronic", "Infectious", "Metabolic", "Viral", "Respiratory", "Cardiovascular", "Bacterial", "Genetic", "Neurological", "Parasitic", "Autoimmune"])
    dalys = st.number_input("DALYs", min_value=0.0, step=1.0)
    treatment = st.selectbox("Treatment Type", ["Vaccination", "Surgery", "Therapy", "Medication"])
    avail = st.selectbox("Availability of Vaccines/Treatment", ["No", "Yes"])
    recovery = st.slider("Recovery Rate (%)", 0.0, 100.0, 1.0)
    improv = st.slider("Improvement in 5 Years (%)", 0.0, 100.0, 1.0)
    beds = st.number_input("Hospital Beds per 1000", min_value=0.0, step=0.1)
    ...
    if st.button("Predict"):
        ...
        input_df = pd.DataFrame([{
            'Age Group': age_group,
            'Gender': gender,
            'Per Capita Income (USD)': income,
            'Education Index': education_index,
            'Prevalence Rate (%)': prevalence,
            'Incidence Rate (%)': incidence,
            'Healthcare Access (%)': access,
            'Doctors per 1000': doctors,
            'Population Affected': affected,
            'Disease Category': category,
            'DALYs': dalys,
            'Treatment Type': treatment,
            'Availability of Vaccines/Treatment': avail,
            'Recovery Rate (%)': recovery,
            'Improvement in 5 Years (%)': improv,
            'Hospital Beds per 1000': beds,
        }])

        # Prediksi dengan model pertama dari yang dipilih
        model = joblib.load("gaussianNB.pkl")
        y_pred = model.predict(input_df)[0]
        y_proba = model.predict_proba(input_df)[0][1]

        risk_label = "High Mortality Risk" if y_pred == 1 else "Low Mortality Risk"
        st.success(f"🩸 Predicted Risk: **{risk_label}** (Probability: {y_proba:.2f})")

    # Stop supaya tidak menjalankan mode lainnya
    st.stop()


elif mode == "Model Analysis & Visualization":
    ...
    for m in models:
        ...
        if hasattr(m, "predict_proba"):
            ...
            st.warning(f"⚠️ Model '{name}' does not support ROC (no predict_proba or decision_function). Skipped.")
            continue
    ...
    # 8. LIME Explanation
    st.subheader("🧠 LIME Explanation (first test sample)")
    ...
    st.subheader("📊 Interactive Visualization by Demographics")
    df_viz = df0.copy().reset_index(drop=True)
    df_viz['Predicted Risk'] = models[0].predict_proba(X)[:, 1]

    st.markdown("#### 🔴 Average Mortality Risk by Age Group")
    ...
    ax1.set_title('Predicted Mortality Risk due to HIV by Age Group')
    ax1.set_ylabel('Average Probability of High Mortality Risk')
    ax1.set_xlabel('Age Group')
    ax1.set_ylim(0, 1)
    st.pyplot(fig1)

    st.markdown("#### 🔥 Risk Heatmap by Gender and Age Group")
    ...
    ax2.set_title('Average Probability of High Mortality Risk due to HIV')
    ax2.set_ylabel('Age Group')
    ax2.set_xlabel('Gender')
    st.pyplot(fig2)

    st.markdown("#### 📦 Risk Distribution by Disease Category")
    fig3, ax3 = plt.subplots(figsize=(12, 6))
    sns.boxplot(data=df_viz, x='Disease Category', y='Predicted Risk', palette='Reds', ax=ax3)
    ax3.set_title('Predicted Probability of High HIV-Related Mortality by Disease Category')
    ax3.set_ylabel('Probability of High Mortality')
    ax3.set_xlabel('Disease Category')
    ax3.set_xticklabels(ax3.get_xticklabels(), rotation=45, ha='right')
    st.pyplot(fig3)

    st.markdown("#### 🔥 Heatmap of Average Risk by Disease Category and Treatment Availability")
    ...
    ax5.set_title("Average Probability of High HIV-Related Mortality by Disease Category and Treatment Availability")
    ax5.set_ylabel("Disease Category")
    ax5.set_xlabel("Availability of Vaccines/Treatment")
    st.pyplot(fig5)

    st.markdown("#### ⏳ Average Mortality Rate Over Time")
    if 'Year' in df_viz.columns:
        fig10, ax10 = plt.subplots(figsize=(10, 6))
        sns.lineplot(data=df_viz, x='Year', y='Mortality Rate (%)', hue='Disease Name', estimator='mean', ax=ax10)
        ax10.set_title('Average Mortality Rate Over Time')
        st.pyplot(fig10)
